...
Validation uses caller-supplied objects and UTC `as_of` only. It does not read the system clock, environment credentials, Git state, network state, hidden files, or provider configuration. Inputs are not mutated. JSON files are limited to 1 MiB, must use UTF-8 with valid Unicode scalar values, reject duplicate keys, and reject non-standard non-finite numbers. Excessive parser depth, object depth, or node count fails closed with a stable error. Linked threat assessments cannot predate their boundary. Error responses use stable codes and never echo secret-bearing values.
