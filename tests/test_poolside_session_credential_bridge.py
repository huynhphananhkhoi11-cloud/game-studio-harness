...
def lease():
    x = {
        ...
        "idempotency_key": "v04-test-0001",
        "canonical_digest": "",
    }
    x["canonical_digest"] = b.canonical_digest(x)
    return x

class PoolsideSessionCredentialBridgeTests(unittest.TestCase):
    def assert_code(self, code, fn, *args, **kwargs):
        with self.assertRaises(b.PoolsideSessionCredentialError) as cm:
            fn(*args, **kwargs)
        self.assertEqual(cm.exception.code, code)
        self.assertNotIn(SECRET, str(cm.exception))

    def test_01_constants(self):
        self.assertEqual(b.CREDENTIAL_PROFILE_ID, "credential-profile:poolside-api-key")
        self.assertEqual(b.ACCOUNT_REF, "account-ref:poolside-owner-account")
...
class PoolsideSessionCredentialBridgeRepairTests(unittest.TestCase):
    def assert_code(self, code, fn, *args, **kwargs):
        with self.assertRaises(b.PoolsideSessionCredentialError) as cm:
            fn(*args, **kwargs)
        self.assertEqual(cm.exception.code, code)
    ...
    def test_repair_01_public_signature_has_no_supplier(self):
        params = _v04_inspect.signature(b.with_secret).parameters
