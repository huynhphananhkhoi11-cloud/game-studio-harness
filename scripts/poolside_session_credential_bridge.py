#!/usr/bin/env python3
"""Session-only Poolside credential bridge for STUDIO-009V-04.
...
SCHEMA_VERSION = "1.0"
CREDENTIAL_PROFILE_ID = "credential-profile:poolside-api-key"
ACCOUNT_REF = "account-ref:poolside-owner-account"
SUBJECT_REF = "provider:poolside"
CAPABILITY = "MODEL_INFERENCE"
PURPOSE = "POOLSIDE_V04_CONNECTED_VALIDATION"
...
SAFE_MESSAGES = {
    "INVALID_LEASE": "Poolside V-04 credential lease metadata is invalid",
    "LEASE_LINEAGE": "credential lease lineage is not authorized for Poolside V-04",
    "LEASE_TIME": "Poolside V-04 credential lease is outside its accepted time window",
    "LEASE_DIGEST": "Poolside V-04 credential lease digest does not match",
    "INTERACTIVE_REQUIRED": "Owner-interactive Poolside key input is required",
    "INVALID_SECRET": "Poolside V-04 session credential is invalid",
    "SECRET_ESCAPE": "Poolside credential material escaped the trusted call boundary",
    "TEST_SECRET_REQUIRED": "deterministic tests require an explicitly synthetic credential supplier",
}

class PoolsideSessionCredentialError(ValueError):
    def __init__(self, code: str):
        self.code = code
        self.safe_message = SAFE_MESSAGES.get(code, "Poolside session credential bridge rejected input")
        super().__init__(self.safe_message)

def _fail(code: str) -> None:
    raise PoolsideSessionCredentialError(code)
...
def with_secret(
    ...
    *,
    ...
    return _consume_secret(
        ...
        consumer,
        as_of=as_of,
        secret_supplier=lambda: getpass.getpass("Poolside API key (hidden, session only): "),
    )
