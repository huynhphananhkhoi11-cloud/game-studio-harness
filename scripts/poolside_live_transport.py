#!/usr/bin/env python3
"""Direct Poolside standalone hosted inference API HTTPS transport for STUDIO-009V-04.
...
SAFE_MESSAGES = {
    "INVALID_REQUEST": "Poolside V-04 request is invalid",
    "SECRET_MATERIAL": "secret-like material is forbidden in Poolside V-04 prompt content",
    "INPUT_LIMIT": "Poolside V-04 estimated input exceeds first-campaign ceiling",
    "OUTPUT_LIMIT": "Poolside V-04 completion ceiling exceeded",
    "REQUEST_TOO_LARGE": "Poolside V-04 request exceeds byte ceiling",
    "INVALID_SECRET": "Poolside V-04 session credential is invalid",
    "NETWORK_ERROR": "Poolside V-04 network operation failed",
    "REDIRECT": "Poolside V-04 redirects are forbidden",
    "AUTH_FAILURE": "Poolside V-04 authentication or authorization failed",
    "PAID_PATH_REQUIRED": "Poolside V-04 encountered a paid or billing-required path",
    "NOT_FOUND": "Poolside V-04 endpoint or exact model was not found",
    "REQUEST_VALIDATION_FAILURE": "Poolside V-04 request validation failed",
    "RATE_LIMIT_OR_CAPACITY": "Poolside V-04 rate limit, capacity, or entitlement prevented execution",
    "PROVIDER_EXECUTION_FAILURE": "Poolside V-04 provider execution failed",
    "UNEXPECTED_STATUS": "Poolside V-04 provider returned an unexpected status",
    "CONTENT_TYPE": "Poolside V-04 response content type is invalid",
    "RESPONSE_TOO_LARGE": "Poolside V-04 response exceeds byte ceiling",
    "MALFORMED_RESPONSE": "Poolside V-04 response is malformed",
    "MODEL_MISMATCH": "Poolside V-04 response model does not match accepted lineage",
    "EXTERNAL_CAPABILITY": "Poolside V-04 response indicates forbidden tool or external capability use",
    "USAGE_INVALID": "Poolside V-04 usage metadata is invalid",
    "TIMEOUT": "Poolside V-04 request timed out",
}

class PoolsideLiveTransportError(ValueError):
    def __init__(self, code: str):
        self.code = code
        self.safe_message = SAFE_MESSAGES.get(code, "Poolside V-04 transport rejected")
        super().__init__(self.safe_message)

def _fail(code: str) -> None:
    raise PoolsideLiveTransportError(code)
...
def _load_json_strict(raw: bytes) -> dict[str, Any]:
    ...
    except PoolsideLiveTransportError:
        raise
...
def perform_request(secret: str, body: dict[str, Any], *, connection_factory=None) -> dict[str, Any]:
    ...
    try:
        ...
        headers = {
            ...
            "User-Agent": "GAME-STUDIO-009V-04/1.0",
        }
